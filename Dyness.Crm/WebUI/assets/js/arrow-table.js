var ArrowTable = (function ($) {
    ...
    ArrowTable.prototype = {
        ...
        focusTarget: function ($target) {
            var trg = $target.find('input');
            var trg1 = $target.find('select');

            if (trg.length > 0) {
                trg.select();
            }
            else if (trg1.length > 0) {
                trg1.focus();
            }
        },
        ...
        bindEvents: function () {
            ...
            this.$table.on('keydown.' + this.options.namespace, this.options.listenTarget, $.proxy(moveEvent, this)).on('keyup.' + this.options.namespace, this.options.listenTarget, keyup);
            this.$table.on('keydown.' + this.options.namespace, 'select', $.proxy(moveEvent, this)).on('keyup.' + this.options.namespace, 'select', keyup);
        },
